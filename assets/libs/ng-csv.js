  ...
  service('CSV', ['$q', function ($q) {
    ...
    this.stringify = function (data, options) {
      ...
      var dataPromise = $q.when(data).then(function (responseData) {
        ...
        // Check if the data are array of nested objects;
        if (angular.isDefined(options.isNested) && options.isNested && typeof options.isNested === 'boolean') {
        ...
        function addKeysColumnHeader(originData) {
            if (angular.isDefined(options.label) && options.label && typeof options.label === 'boolean') {
                ...
                encodingArray = [];
                angular.forEach(originData[0], function(value, label) {
      ...
      controller: [
        ...
        function ($scope, $element, $attrs, $transclude) {
          ...
          function getBuildCsvOptions() {
            ...
            if (angular.isDefined($attrs.isNested)) options.isNested = $scope.$eval($scope.isNested);
